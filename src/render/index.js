...
export default function renderToString (vnode, context, opts, inner, isSvgMode) {
  ...
  let indentChar = typeof pretty === 'string' ? pretty : '\t';
  ...
  if (typeof nodeName === 'function') {
    ...
    if (opts.shallow && (inner || opts.renderRootComponent === false)) {
      ...
      const rendered = nodeName(props, context);
  ...
  if (html) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (!falsey(child)) {
        ...
        let ret = renderToString(child, context, opts, true, childSvgMode);
        if (!hasLarge && pretty && isLargeString(ret)) {
          hasLarge = true;
        }
        pieces.push(ret);
      }
...
function getFallbackComponentName (component) {
  ...
  let name = (str.match(/^\s*function\s+([^( ]+)/) || EMPTY)[1];
  if (!name) {
